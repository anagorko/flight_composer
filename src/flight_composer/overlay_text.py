...
from flight_composer.flight_data import GoProMetadata, GoProTelemetry, IGCTelemetry
...
def get_flight_map_overlay_text(igc_data: IGCTelemetry) -> str | None:
    ...
    glider_type = meta.glider_type
    glider_id = meta.glider_id
...
def get_gopro_overlay_text(gopro_data: GoProTelemetry) -> str | None:
    ...
    fps = video.frame_rate_hz
    if res and fps:
        parts.append(_format_resolution(res, fps))

    # 3. Lens / FOV
    settings = meta.settings
    fov_label = _format_fov(settings.field_of_view, settings.lens_projection)
    if fov_label:
        parts.append(fov_label)

    # 4. Stabilisation
    stab_label = _format_stabilisation(
        settings.electronic_stabilization_on,
        settings.electronic_stabilization,
    )
    if stab_label:
        parts.append(stab_label)

    # 5. Digital Zoom (affects stabilisation behaviour)
    if settings.digital_zoom_on:
