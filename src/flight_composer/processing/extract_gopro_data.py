"""
GoPro telemetry stream extraction — parse a GoPro MP4 file into a ``GoProTelemetry`` object.

Signature: ``extract_gopro_data(mp4_path: Path) -> GoProTelemetry``

Takes the path to a GoPro ``.mp4`` file and returns a fully populated ``GoProTelemetry`` object.

The flight tag is derived from ``mp4_path.stem`` (e.g.
``20250508_gopro.mp4`` → ``"20250508_gopro"``).  Both DataFrames are saved
as Parquet files (pyarrow engine) to ``config.DIR.TELEMETRY``.
...
#### 2.3 Output: GPS-rate trajectory DataFrame (Parquet)

The parsed GPS-rate data is assembled into a pandas DataFrame and saved as a
Parquet file at ``config.DIR.TELEMETRY / f"{flight_tag}_mp4.parquet"``. The
path is stored in ``gopro_telemetry.data_path``.
...
This DataFrame is the primary input for pipeline phases 1–6 (trajectory reconstruction, cleaning, alignment, fusion, kinematics). The `timestamp_s` column contains seconds since recording start; to obtain absolute UTC times, add ``metadata.gps_start_utc`` from the ``GoProMetadata`` object (§2.5).

#### 2.4 Output: high-frequency IMU / orientation DataFrame (Parquet)

The parsed high-frequency IMU and orientation data is assembled into a pandas
DataFrame and saved as a Parquet file at
``config.DIR.TELEMETRY / f"{flight_tag}_mp4_hf.parquet"``. The path is stored
in ``gopro_telemetry.hf_imu_path``.
...
This DataFrame is the primary input for phase 7 (high-frequency jitter extraction). CORI is the preferred orientation source; GYRO is used as a fallback or for validation.

> **Note on IMU axis order:** GoPro IMU axis conventions vary by camera model (e.g. HERO5: Z,X,Y; HERO6+: Y,-X,Z for ACCL). The extraction code must detect the camera model from GPMF header metadata (DVNM) and remap axes to a consistent body-frame convention (X-Forward, Y-Right, Z-Down) before writing the DataFrame.

#### 2.5 Output: GoProMetadata object

Recording-level metadata is stored directly in the ``gopro_telemetry.metadata``
field as a :class:`GoProMetadata` pydantic model — no external JSON file is
written.  Data is extracted from three sources:

- MP4 container tags (via ``ffprobe``)
- ffprobe stream properties (video resolution, codec, frame rate, audio format)
- GoPro "Global Settings" GPMF payload stored in the MP4 ``moov > udta > GPMF`` atom
...
| ``gps_start_utc`` | GPMF ``GPSU`` (first payload) | UTC ``datetime`` of the first GPS sample |
...
##### 2.5.2 ``video`` sub-object (``GoProVideoConfig``)
...
##### 2.5.3 ``audio`` sub-object (``GoProAudioConfig``)
...
##### 2.5.4 ``settings`` sub-object (``GoProSettings``) — GoPro camera settings
...
| Field | GPMF FourCC | Example | Description |
|-------|-------------|---------|-------------|
...
The ``GoProMetadata`` and its sub-objects use ``extra="allow"``, so additional
fields from ffprobe or GPMF that are not explicitly declared are preserved as
extra attributes.

> **Max Lens Mod detection:** The ``lens_projection`` settings field indicates
> the lens type. Standard GoPro lens = ``"GPRO"``; Max Lens Mod = ``"MLNS"``.

> **Stabilization detection:** ``electronic_stabilization_on`` is ``True`` when
> HyperSmooth is active. The ``electronic_stabilization`` extra field gives the
> exact level (e.g. ``"HS High"``, ``"HS Boost"``, ``"Standard"``).

The ``metadata.gps_start_utc`` field is the authoritative absolute time reference. To convert any ``timestamp_s`` value from the trajectory or HF IMU DataFrames to UTC: ``utc = gps_start_utc + timedelta(seconds=timestamp_s - first_gps_timestamp_s)``, where ``first_gps_timestamp_s`` is the earliest ``timestamp_s`` in the GPS-rate DataFrame.
...
def extract_gopro_data(mp4_path: Path) -> GoProTelemetry | None:
    """Parse a GoPro MP4 file and return a fully populated :class:`GoProTelemetry`.
    ...
    -------
    GoProTelemetry
        A new object with ``mp4_path``, ``metadata``, ``data_path``,
        and ``hf_imu_path`` populated.  DataFrames are saved as Parquet
        (pyarrow engine) to ``config.DIR.TELEMETRY``.  The flight tag
        used for output filenames is derived from ``mp4_path.stem``.
    """
    flight_tag = mp4_path.stem
    ...
    # ------------------------------------------------------------------
    # Flight metadata → GoProMetadata object
    # ------------------------------------------------------------------
    ...
    # Top-level identifiers from settings (promoted into metadata extras)
    ...
    metadata = GoProMetadata.model_validate(
        {
            "device_name": payloads[0].device_name if payloads else "unknown",
            "gps_start_utc": gps_start_utc,
            "creation_time": mp4_meta.get("creation_time"),
            "duration_s": mp4_meta.get("duration_s"),
            # Sub-objects — constructed from ffprobe / GPMF dicts
            "video": mp4_meta.get("video") or {},
            "audio": mp4_meta.get("audio") or {},
            "settings": gopro_settings if gopro_settings else {},
    ...
    # ------------------------------------------------------------------
    # GPS-rate trajectory DataFrame → Parquet
    # ------------------------------------------------------------------
    gps_df = _build_gps_rate_df(payloads)
    data_path: Path | None = None

    if not gps_df.empty:
        data_path = output_dir / f"{flight_tag}_mp4.parquet"
        gps_df.to_parquet(data_path, index=False, engine="pyarrow")
        parquet_size_kb = data_path.stat().st_size / 1024
        ts = gps_df["timestamp_s"]
        span = ts.iloc[-1] - ts.iloc[0]
        logger.info(
            "Wrote GPS Parquet:  %s  (%d rows, %.1f s, %.0f KiB)",
            data_path.name,
            len(gps_df),
            span,
            parquet_size_kb,
        ...
        logger.warning("No GPS5 data found — skipping GPS-rate Parquet")

    # ------------------------------------------------------------------
    # High-frequency IMU / orientation DataFrame → Parquet
    # ------------------------------------------------------------------
    hf_df = _build_hf_imu_df(payloads)
    hf_imu_path: Path | None = None

    if not hf_df.empty:
        hf_imu_path = output_dir / f"{flight_tag}_mp4_hf.parquet"
        hf_df.to_parquet(hf_imu_path, index=False, engine="pyarrow")
        parquet_size_mb = hf_imu_path.stat().st_size / (1024 * 1024)
        ts = hf_df["timestamp_s"]
        span = ts.iloc[-1] - ts.iloc[0]
        logger.info(
            "Wrote HF Parquet:   %s  (%d rows, %.1f s, %.1f MiB)",
            hf_imu_path.name,
            len(hf_df),
            span,
            parquet_size_mb,
        ...
        logger.warning("No ACCL data found — skipping HF IMU Parquet")
